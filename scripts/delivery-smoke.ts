...
function resolveConfig(): ScriptConfig {
  ...
  const adminPassword =
    ...
    "";
  const accountEmail =
    parseFlagValue("--account-email") ??
    process.env.DELIVERY_SMOKE_ACCOUNT_EMAIL ??
    adminEmail;
  const accountPassword =
    parseFlagValue("--account-password") ??
...
async function main() {
  ...
  try {
    ...
    if (accountSession !== adminSession) {
      await runStep(results, "auth:account-login", async () => {
        const payload = await login(accountSession, config.accountEmail, config.accountPassword, "account");
        return {
          details: `email=${payload?.email ?? config.accountEmail}`,
          value: payload,
        };
      });
    } else {
    ...
    await runStep(results, "driver:start", async () => {
      ...
      return {
        details: `status=${payload.run.status}`,
        value: payload.run,
      };
    });
    ...
    await runStep(results, "driver:stop-complete", async () => {
      const result = await adminSession.request<{ run?: DeliveryRunSummary }>(
        `/api/driver/run/${driverToken}/stops/${stopToComplete.id}/complete`,
        {
          method: "POST",
          json: {
            result: "DELIVERED",
