  ...
  return (
    <>
      ...
      <section className="section">
        ...
        <div className="table-wrap"><table><thead><tr><th>{language === "fr" ? "Produit" : "Product"}</th><th>{language === "fr" ? "Variation" : "Change"}</th><th>{language === "fr" ? "Raison" : "Reason"}</th><th>{language === "fr" ? "Commande" : "Order"}</th><th>{language === "fr" ? "Date" : "Date"}</th></tr></thead><tbody>{inventoryItems.map((movement) => (<tr key={movement.id}><td>{movement.productName}</td><td className={movement.quantityChange >= 0 ? "ok" : "err"}>{movement.quantityChange > 0 ? `+${movement.quantityChange}` : movement.quantityChange}</td><td>{movement.reason}</td><td>{movement.orderNumber ?? "-"}</td><td>{formatDateTime(movement.createdAt)}</td></tr>))}</tbody></table></div>
      </section>
    </>
  );
}
