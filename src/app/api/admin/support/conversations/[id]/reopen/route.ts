...
import { reopenSupportConversation } from "@/lib/support";

export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    const admin = await requireAdmin();
    const { id } = await context.params;
    const conversation = await reopenSupportConversation(id, admin);
    ...
    return jsonError("Failed to reopen support conversation", 500);
  }
}
