    ...
    try {
      ...
      if (!response.ok) {
        setError(
          data.error ??
            (language === "fr"
              ? "Impossible de preparer l'abonnement pour le moment."
      ...
      setError(
        language === "fr"
          ? "Une erreur est survenue pendant la preparation de l'abonnement."
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
        ...
        <strong>{language === "fr" ? "Abonnement recurrent" : "Recurring subscription"}</strong>
      </div>

      <p className="small" style={{ marginBottom: 16 }}>
        {language === "fr"
          ? "Choisis la frequence qui te convient. Tu pourras gerer l'abonnement depuis ton compte."
      ...
      {status === "success" ? (
        <div className="support-lite-card" style={{ padding: 20, marginBottom: 16 }}>
          <p className="support-lite-card__eyebrow" style={{ margin: 0 }}>
            {language === "fr" ? "Abonnement confirme" : "Subscription confirmed"}
          </p>
          <h3 className="support-lite-card__title" style={{ margin: "0.25rem 0 0.5rem" }}>
            {language === "fr"
              ? `Merci, l'abonnement a ${productName} est pret.`
              : `Thank you, your ${productName} subscription is ready.`}
          </h3>
          <p className="small support-lite-card__text" style={{ margin: 0 }}>
            {language === "fr"
              ? "Le paiement Stripe est accepte. La synchronisation finale peut prendre quelques secondes."
              : "Stripe accepted the payment. Final syncing can take a few seconds."}
          </p>
      ...
      {status === "pending" ? (
        <div className="auth-alert auth-alert--ok" style={{ marginBottom: 16 }}>
          <span>✓</span>{" "}
          {language === "fr"
            ? "Stripe a bien recu la demande. La confirmation finale peut prendre quelques secondes."
            : "Stripe received your request. Final confirmation can take a few seconds."}
        </div>
      ) : null}
      ...
      {status === "cancelled" ? (
        <div className="auth-alert auth-alert--err" style={{ marginBottom: 16 }}>
          <span>⚠️</span>{" "}
          {language === "fr"
            ? "Le paiement d'abonnement a ete interrompu. Tu peux reprendre quand tu veux."
      ...
      {!embeddedCheckout ? (
        <button className="btn btn-full" disabled={!selectedInterval || loading} onClick={() => void subscribe()}>
          {loading
            ? "..."
            : language === "fr"
              ? "Preparer le paiement integre"
              : "Prepare inline payment"}
        </button>
      ) : (
        <StripeInlineCheckout
          ...
          description={
            language === "fr"
              ? `Ton abonnement ${selectedIntervalLabel.toLowerCase()} a ${productName} se confirme ici, sans quitter la page sauf si Stripe l'exige.`
