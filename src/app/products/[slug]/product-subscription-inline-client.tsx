...
export function ProductSubscriptionInlineClient({
  ...
  const subscribe = async () => {
    ...
    try {
      ...
      const data = (await res.json()) as SubscriptionCheckoutResponse & { error?: string }
      if (!res.ok) {
        setError(
          data.error ??
            (language === 'fr'
  ...
  return (
    ...
    >
      ...
      <p className="small" style={{ marginBottom: 16 }}>
        {language === 'fr'
          ? 'Choisis la fréquence de livraison qui te convient. Tu peux annuler depuis ton compte au bon moment.'
          : 'Choose the delivery frequency that works for you. You can cancel from your account whenever the timing is right.'}
      </p>
      ...
      {status === 'success' ? (
        <div className="support-lite-card" style={{ padding: 20, marginBottom: 16 }}>
          ...
          <p className="small support-lite-card__text" style={{ margin: 0 }}>
            {language === 'fr'
              ? "Le paiement Stripe est accepté. La synchronisation finale peut prendre quelques secondes, puis tu retrouveras tout dans ton compte."
              : 'Stripe accepted the payment. Final syncing can take a few seconds, then everything will appear in your account.'}
          </p>
      ...
      {status === 'pending' ? (
        <div className="auth-alert auth-alert--ok" style={{ marginBottom: 16 }}>
          <span>✓</span>{' '}
          {language === 'fr'
            ? "Stripe a reçu ta demande. La confirmation finale de l'abonnement peut prendre quelques secondes."
            : 'Stripe received your request. Final subscription confirmation can take a few seconds.'}
        </div>
      ) : null}
