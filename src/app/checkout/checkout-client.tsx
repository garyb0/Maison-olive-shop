...
export function CheckoutClient({
  ...
  useEffect(() => {
    if (searchParams.get("cancelled") === "1") {
      ...
      setError(
        language === "fr"
          ? "Le paiement Stripe a été annulé. Vous pouvez reprendre le checkout quand vous voulez."
          : "Stripe checkout was cancelled. You can resume checkout anytime.",
      );
  ...
  const submitOrder = async () => {
    if (!user && paymentMethod !== "STRIPE") {
      setError(
        language === "fr"
          ? "Le paiement manuel nécessite un compte. Utilisez Stripe en invité ou connectez-vous."
          : "Pay on delivery requires an account. Use guest Stripe checkout or sign in.",
      );
      return;
    }
  ...
  const handleStripeInlineSuccess = (session: { status: { type: "open" | "expired" | "complete"; paymentStatus?: "paid" | "unpaid" | "no_payment_required" } }) => {
    ...
    setMessage(
      language === "fr"
        ? "Stripe a bien reçu la confirmation. La finalisation peut prendre quelques secondes."
        : "Stripe received the confirmation. Final processing can take a few seconds.",
    );
  };
                  ...
                  <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
                    ...
                    <div style={{ minWidth: 0, flex: "1 1 280px" }}>
                      ...
                      <p className="small" style={{ margin: "6px 0 0", color: "#6f624d" }}>
                        {language === "fr"
                          ? "Tu peux commander sans compte avec Stripe. Le paiement manuel reste réservé aux clients connectés."
                          : "You can place your order without an account using Stripe. Pay on delivery stays reserved for signed-in customers."}
                      </p>
                    </div>
                          ...
                          {calendarDays.map((day) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <span>{label}</span>
                                <small>{availableDay.periodCount} {language === "fr" ? "période(s)" : "period(s)"}</small>
                              </button>
                            );
                          })}
                        </div>
            ...
            <section className="section checkout-section-card">
              ...
              <div className="checkout-payment-methods">
                ...
                <label className={`checkout-payment-option${paymentMethod === "STRIPE" ? " checkout-payment-option--active" : ""}`}>
                  ...
                  <div className="checkout-payment-option-text">
                    ...
                    <span className="checkout-payment-option-desc">
                      {language === "fr"
                        ? "Paiement Stripe intégré directement dans cette page, avec redirection seulement si le moyen de paiement l’exige."
                        : "Stripe payment stays on this page, with a redirect only if the payment method requires it."}
                    </span>
                  </div>
              ...
              {!user ? (
                <p className="small" style={{ marginTop: 10, marginBottom: 0, color: "#6f624d" }}>
                  {language === "fr"
                    ? "En invité, Stripe est l’option recommandée. Ton paiement est confirmé immédiatement et ton courriel sert au reçu."
                    : "As a guest, Stripe is the recommended option. Your payment is confirmed immediately and your email is used for the receipt."}
                </p>
              ) : null}
              ...
              {paymentMethod === "STRIPE" ? (
                <p className="small" style={{ marginTop: 12, marginBottom: 0, color: "#6f624d" }}>
                  {stripeSession
                    ? language === "fr"
                      ? "Le formulaire Stripe est maintenant ancre dans le resume de commande a droite pour une validation plus simple."
                      : "The Stripe form is now anchored in the order summary on the right for a cleaner confirmation flow."
                    : language === "fr"
                      ? "Quand tu prepares le paiement, le formulaire Stripe s'ouvre directement dans le resume de commande."
                      : "When you prepare the payment, the Stripe form opens directly inside the order summary."}
                </p>
              ) : null}
            </section>
            ...
            <div className="checkout-summary-card">
              ...
              {stripeReturnNotice === "paid" ? (
                <div className="auth-alert auth-alert--ok">
                  <span>✓</span>{" "}
                  {language === "fr"
                    ? "Paiement Stripe confirmé. Le reçu et la commande ont été validés."
                    : "Stripe payment confirmed. Your receipt and order were validated."}
                </div>
              ) : null}
              ...
              {stripeReturnNotice === "pending" ? (
                <div className="auth-alert">
                  <span>…</span>{" "}
                  {language === "fr"
                    ? "Stripe a bien reçu le retour. La confirmation finale peut prendre quelques secondes."
                    : "Stripe received the return. Final confirmation can take a few seconds."}
                </div>
              ) : null}
              ...
              {paymentMethod === "STRIPE" && stripeSession ? (
                <div style={{ display: "grid", gap: 12 }}>
                  <div className="auth-alert auth-alert--ok" style={{ marginTop: 0 }}>
                    <span>✓</span>{" "}
                    {language === "fr"
                      ? "Ton paiement est pret ici. Verifie la carte ci-dessous et confirme-la sans quitter la page."
                  ...
                  <StripeInlineCheckout
                    ...
                    submitLabel={language === "fr" ? "Confirmer le paiement Stripe" : "Confirm Stripe payment"}
                    loadingLabel={language === "fr" ? "Confirmation..." : "Confirming..."}
                    headline={language === "fr" ? "Confirme ton paiement ici" : "Confirm your payment here"}
                    description={
                      language === "fr"
                        ? "Ton panier est reserve pour cette tentative. Si Stripe exige une verification externe, tu reviendras automatiquement sur cette page."
                        : "Your cart is reserved for this attempt. If Stripe requires an external verification, you will return to this page automatically."
                    }
