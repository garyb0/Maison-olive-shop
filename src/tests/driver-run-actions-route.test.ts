...
describe("driver run action routes", () => {
  ...
  it("marque un arret chauffeur comme livre", async () => {
    ...
    const { POST } = await import("@/app/api/driver/run/[token]/stops/[stopId]/complete/route");

    const response = await POST(
      new Request("http://localhost:3101/api/driver/run/token_1/stops/stop_1/complete", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          result: "DELIVERED",
