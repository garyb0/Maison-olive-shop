...
function normalizeStripeMessage(message: string, language: "fr" | "en") {
  ...
  if (normalized.includes("customer_email") && normalized.includes("checkout session")) {
    return language === "fr"
      ? "Le courriel est deja verrouille pour cette tentative. Recharge la page et relance le paiement Stripe."
      : "The email is already locked for this attempt. Refresh the page and start the Stripe payment again.";
  }

  if (normalized.includes("phone_number_collection.enabled")) {
    return language === "fr"
      ? "Le telephone est deja gere par ton formulaire de livraison. Recharge la page et relance le paiement Stripe."
      : "The phone number is already handled by your delivery form. Refresh the page and start the Stripe payment again.";
  }
  ...
  if (checkoutState.type === "loading") {
    return (
      <div className="small" style={{ color: "#6f624d" }}>
        {language === "fr" ? "Chargement du formulaire Stripe..." : "Loading the Stripe form..."}
      </div>
    );
  }
    ...
    try {
      ...
      onError(
        error instanceof Error
          ...
          : language === "fr"
            ? "La confirmation Stripe a echoue."
  ...
  if (!stripeClientReady) {
    return (
      <div className="auth-alert auth-alert--err">
        ...
        {errorMessage ??
          (language === "fr"
            ? "Stripe n'est pas pret cote navigateur. Ajoute NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY."
    ...
    >
      <div style={{ display: "grid", gap: 6, marginBottom: isSummaryVariant ? 10 : 18 }}>
        <p className="support-lite-card__eyebrow" style={{ margin: 0 }}>
          {language === "fr" ? "Paiement Stripe integre" : "Embedded Stripe payment"}
        </p>
